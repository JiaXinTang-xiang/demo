...
// 本地函数声明
static void TIM4_Init(uint16_t PSC, uint16_t ARR, FunctionalState NewState);
...
void SR04_Init(void)
{
    ...
    TIM4_Init(72, 30000, DISABLE);                       // TIM6初始化，分频值:72, 重装载值：30000，即：每个脉冲0.0000001秒(1MHz: 72000000/7200), 每30000次脉冲产生一次中断(0.03秒：30000*0.000001);
...
#include "stm32f10x_tim.h"
    ...
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);  // 开启TIM6时钟,即内部时钟CK_INT=72M
    TIM_Cmd(TIM2, DISABLE);                               // 在配置前，先关闭计数器
    ...
    TIM_Cmd(TIM2, NewState);                              // 是否使能计数器(使能了就会立即开始工作)
...
{
    TIM2->SR &= (uint16_t)~0x01;   // 清0更新中断标志位
...
static void SR04_Stop(void)
{
    TIM2->CR1 = 0;                 // 是否使能计数器(使能了就会立即开始工作)
    TIM2->CNT = 0;
    TRIG_0 ;
}
...
float SR04_GetCM(void)
{
    ...
    TRIG_0;
    /********2.启动TIM，记录ECHO高电平时长**********/
    TIM2->CNT = 0;        // 狗血日常：调试过程无意中删了这一行，然后剧情他妈的精彩。排查一轮后，心力交瘁躺平发呆，刚眯上眼才来的灵感：是不是CNT计数器没清0！！
    while (ECHO == 0);    // 等待高电平开始(SR04工作后，ECHO引脚将返回声波来回的等长高电平时长）
    TIM2->CR1 |= 0x01;    // TIM开始工作
    while (ECHO == 1);    // 等待高电平结束
    TIM2->CR1 = 0x00;     // TIM停止工作
    /********3.计算距离,单位:CM**********/
    xSR04.CM = TIM2->CNT * 0.0173;  // 摄氏25度时音速每秒346米，因为TIM初始化成1us脉冲，换算成：0.0346厘米/1us; Echo引脚的高电平时长，是声波从发出到接收的时间，所以要除以2
